...
public class OrderServiceImpl implements OrderService {
    ...
    public List<OrderDTO> findAllByUserId(Integer userId, Pagination pagination) {
        int count = repository.countUserOrders(userId);
        if (count == 0) {
            return emptyList();
        }
        ...
        return repository.findAllByUserId(userId, pagination)
                .stream()
                .map(this::convertToDto)
                .map(this::addSelfRelLink)
                .collect(Collectors.toList());
    ...
    @Override
    public void deleteById(Integer userId, Integer orderId) throws IllegalAccessException {
        OrderDTO order = findById(orderId);
        if (!order.getUser().getId().equals(userId)) {
            throw new IllegalAccessException("This order does not belong to this user");
        }
        deleteById(orderId);
    }
    ...
    public Integer countUserOrders(Integer userId) {
        return repository.countUserOrders(userId);
    }
