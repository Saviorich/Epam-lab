...
public class OrderController implements PageableResourceController<OrderDTO> {
    ...
    @PostMapping
    public ResponseEntityStatus makeOrder(@RequestBody OrderDTO order) {
        log.debug("Order passed in POST request: {}", order);
        service.save(order);
        return OK;
    }

    @DeleteMapping("/{id}")
    public ResponseEntityStatus deleteById(@PathVariable Integer id) {
